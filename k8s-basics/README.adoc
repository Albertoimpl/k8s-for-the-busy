...
A service is is an abstraction which defines a set of Pods and a policy to access them
...
ReplicaSets are used to  to maintain the number and running state of pods.
...
It contains all the replicaset provided to us and also ensures that any update we make to our pods do not cause downtime.
...
We came a long way but now we have all this amazing capabilities.
But there are a couple of ways go get all this yaml generated for us.
The easier one:
